import { useChartDataComposer } from "@/plugins/charts/widgets/api/ChartdataComposer";
import type {
    AxisData,
    Composer,
    Selector,
...
const useChartDataComposer = () => {
    ...
    const removeComposer = (composer: Composer<Selector>) => {
        const index = composers.value.indexOf(composer);
        if (index != -1) {
    ...
    const getDataForMergedAxisX = (): Ref<AxisData> | ComputedRef<AxisData> => {
        return computed(() => {
            ...
            return {
                data: data,
                title: composers.value[0] ? composers.value[0].title : "",
            };
        });
    };
    const getDataForAxesY = ():
        | Ref<Array<AxisData>>
        | ComputedRef<Array<AxisData>> => {
        return computed(() => {
            let data: AxisData[] = [];
            composers.value.forEach((composer) => {
