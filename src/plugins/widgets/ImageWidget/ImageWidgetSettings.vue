...
import { v4 } from "uuid";
import { ref } from "vue";
import type { IImageSettings } from "@/types/Widgets";
    ...
    <va-collapse v-model="opened.widgetSection" header="Image widget settings">
        <div class="settings-container">
            ...
            <div class="image-list-container">
                <div v-for="(image, index) in widgetSettings.images" :key="image.id" class="image-settings-container">
                    <va-input v-model="image.url" :label="t('ImageWidget.imageUrl')"
                        class="image-settings-remove-input" />
                    <va-button @click="() => widgetSettings.images.splice(index, 1)
                        " icon="clear" class="image-settings-remove-button" />
                    <!-- {{ image.url }} -->
                </div>
            </div>
            <va-select v-model="widgetSettings.imagesSettings.fit" :label="t('ImageWidget.imageFit')"
                :options="['Cover', 'Contain', 'Stretch', 'Fill', 'None']" teleport=".settings-container">
            </va-select>
            <va-input v-model="widgetSettings.imagesSettings.diashowInterval"
