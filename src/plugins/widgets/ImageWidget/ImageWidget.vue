...
onMounted(() => {
    ...
    if (!config.value.imagesSettings) {
        config.value.imagesSettings = {
            diashowInterval: 0,
            fit: "None",
        };
    }
    ...
    <template v-else>
        <div class="galery-container">
            <div class="button-prev">
                <va-button @click="toPrev()" icon="chevron_left" preset="plain" text-color="#ffffff"
                    :disabled="currentImage === 0"></va-button>
            </div>
            <div class="galery-viewport" :style="`transform: translateX(-${100 * currentImage}%)`">
                <div v-for="(image, i) in config.images" :key="image.id" class="galery-image"
            ...
            <div class="button-next">
                <va-button @click="toNext()" icon="chevron_right" text-color="#ffffff"
                    :disabled="currentImage === config.images?.length - 1" preset="plain"></va-button>
            </div>
        </div>
    </template>
</template>
...
.image {
    width: 100%;
    height: 100%;
    object-fit: v-bind(config.imagesSettings?.fit);
}
