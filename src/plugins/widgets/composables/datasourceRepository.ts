...
export function useDatasourceRepository<T extends keyof DataMap>(
    dataSourceId: Ref<string>,
    type: T,
): IVueDatasourceRepository<T> {
    const datasourceRepository = container.get<DatasourceRepository>(
        SERVICE_IDENTIFIER.DatasourceRepository,
    );
