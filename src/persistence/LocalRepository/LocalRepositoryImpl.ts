...
export default class LocalRepositoryImpl extends BaseRepository implements WritableRepository{
    ...
    async findAll(): Promise<Entity[]> {
        ...
        Object.keys(localStorage).filter((e) => e.startsWith('__DAANS_FILE_')).forEach((e, key) => {
            try {
    ...
    create(e:Entity): Promise<any> {
        ...
        const name = '__DAANS_FILE_' + e.uri.pathname.replace('.json','').replace('/','');
